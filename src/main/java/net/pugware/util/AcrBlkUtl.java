...
public enum AcrBlkUtl {;
   ...
   public static boolean canPlace(BlockState state, BlockPos pos) {
      return Pugware.MC.world.canPlace(state, pos, (ShapeContext)null);
   }

   public static boolean hasBlock(BlockPos pos) {
      return !Pugware.MC.world.getBlockState(pos).isAir();
   }
   ...
   public static Block getBlock(BlockPos pos) {
      return Pugware.MC.world.getBlockState(pos).getBlock();
   }

   public static BlockState getBlockState(BlockPos pos) {
      return Pugware.MC.world.getBlockState(pos);
   }
   ...
   public static BlockHitResult clientRaycastBlock(BlockPos pos) {
      return Pugware.MC.world.raycastBlock(RotationUtils.getEyesPos(), RotationUtils.getClientLookVec().multiply(6.0D).add(RotationUtils.getEyesPos()), pos, getBlockState(pos).getOutlineShape(Pugware.MC.world, pos), getBlockState(pos));
   }
   ...
   public static boolean isBlockReachable(BlockPos pos, double reach) {
      ...
      VoxelShape shape = state.getOutlineShape(Pugware.MC.world, pos);
      if (shape.isEmpty()) {
         shape = fullCube();
      }
   ...
   public static boolean rightClickBlock(BlockPos pos) {
      ...
      if (shape.isEmpty()) {
         ...
         ActionResult result2 = Pugware.MC.interactionManager.interactBlock(Pugware.MC.player, Hand.MAIN_HAND, new BlockHitResult(hitVecs[side.ordinal()], side, pos, false));
